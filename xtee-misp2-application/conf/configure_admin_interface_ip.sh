...
if ! (echo "$action" | grep -Eq "$action_regex")
then
...
then
	...
	if ! (is_valid_ip_conf $new_admin_ip)
	then
...
then
	...
	if echo "$apache_conf_admin_ip" | grep -Eq "[,\s]"
	then
	...
	if [ "$ssh_remote_ip" != "" ] && [ "$ssh_remote_ip" == "$logged_in_remote_ip" ] && \
		is_valid_ip_conf "$ssh_remote_ip"
	then
	...
	then
		...
		if echo "$apache_conf_admin_ip" | grep -q "$new_admin_ip"
		then
	...
	# If IP-s are overwritten
		...
		if [ "$new_admin_ip" == "$apache_conf_admin_ip" ] 
		then
